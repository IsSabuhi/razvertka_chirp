...
- запускает `chirpstack-v3-to-v4`.

Если мигратор падает с `idx_user_email`, в БД `chirpstack` уже есть конфликтующие пользователи (обычно следы прошлой попытки). Нужна очистка конфликтных данных в v4 или пересоздание пустой БД `chirpstack` (после бэкапа), затем повтор миграции.
